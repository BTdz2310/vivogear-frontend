...
const getDevice = async (device) => {
    ...
    quickSpec['Display size'] = displaySize;
    quickSpec['Display resolution'] = displayRes;
    quickSpec['Camera pixels'] = cameraPixels;
    quickSpec['Video pixels'] = videoPixels;
    quickSpec['RAM size'] = ramSize;
    quickSpec['Chipset'] = chipset;
    quickSpec['Battery size'] = batterySize;
    quickSpec['Battery type'] = batteryType;
    ...
    specNode.each((i, el) => {
        ...
        if(category){
            ...
            specN.each((index, ele) => {
                const name = $('td.ttl', ele).text();
                const value = $('td.nfo', ele).text();
                detailSpec[category][name] = value;
            });
        }
...
const getImages = async (device) => {
    const arr = device.split('-');
    // console.log(arr)
    const html = await getDataFromUrl(`/${arr[0]}-pictures-${arr[1]}.php`);
    
    // console.log(`/${arr[0]}-pictures-${arr[1]}.php`);
    // // console.log(html)
    // console.log(typeof html)
    
    const namePhone = arr[0].split('_');
    let realName = '';
    namePhone.forEach((ele, ind)=>{
        if(ind===0) realName += `${ele}`
        else realName += ` ${ele}`
    })
    // console.log(realName)
    const $ = cheerio.load(html);
    const imgs = $("div#pictures-list img");

    const imgNe = [];
    for(let i=0; i<(imgs.length<20?imgs.length:20); i++){
        // console.log(imgs[i].attribs)
        imgNe.push(imgs[i].attribs.src?imgs[i].attribs.src:imgs[i].attribs['data-src'])
    }
    // imgs.map(img=>{
